...
| Operator console (`app/`) | Bench-verified | runs on the Jetson, relays EO + radar + detector + recorder, record/replay exercised on-device | the deploy procedure is now written down ([`app/docs/DEPLOY.md`](../app/docs/DEPLOY.md)) including the `web_assets.h` regeneration trap. The tracker integration was run end-to-end against a live `trackerd` on the box 2026-07-21 (tracks through the proxy, engage → lock). **The fusion integration (`/fus/stream`, fusion-driven target list, fused symbology) is verified only against a stand-in daemon serving the contract — never against the real `fusiond`, which is not yet built on the Jetson** |

...
static void  (*g_scene_cb)(int, int, double, double, void *) = NULL;
...
static void *client(void *arg) {
    ...
    if (!strncmp(req, "GET /scene", 10)) {
        ...
        double rate = -1.0;
        { const char *h = strstr(req, "rate="); if (h) rate = atof(h + 5); }
        if ((on >= 0 || reset || hl >= 0.0 || rate > 0.0) && g_scene_cb)
            g_scene_cb(on, reset, hl, rate, g_scene_user);
        pthread_mutex_lock(&g_lock);

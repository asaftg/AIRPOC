...
- **Target boxes**: one per **confirmed track** (the tracker confirms, coasts
  through short dropouts, and park-holds a moved-then-stopped target — see
  [`TUNING.md`](TUNING.md)). Arrow = 1 s of velocity; label is
  `R#<id>  <speed> m/s · <range> m`. IDs are transient radar-track ids (`R#`,
  stable frame-to-frame) — fusion assigns global ids later.
- **HUD** (top-left) = profile, Hz, point/target counts, dropped-frame count.

The previewer draws only the current frame — no trails; persistence through a
dropout is the tracker's coast/park, not a display effect.

...
| `GET /export?sids=<a,b,..\|all>&tier=meta\|display\|full` | stream the selected sessions as one `.tar` download. **meta** = annotations+thumbs+radar (no video); **display** = a playable `native.mp4` movie + metadata (raw channels excluded); **full** = everything (raw channels + movie). The movie is transcoded on the fly if not already cached. Sids validated |

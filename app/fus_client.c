/* Fusion SSE client — subscribes to the fusion daemon's /stream (fusion/ module, fusiond on
 * :8096) and keeps the latest fused target message. See fus.h. Reconnects forever; reports
 * connected=0 while unreachable — the console must run fine with no fusion, falling back to the
 * per-sensor lists. Same structure as trk_client.c / det_client.c / radar_client.c. */
#define _GNU_SOURCE
#include "fus.h"
#include <arpa/inet.h>
#include <errno.h>
...
#define ACC_CAP   (256 * 1024)   /* fused target messages carry the joined target set */
...
static char            g_fstats[STATS_CAP] = "";
static char            g_host[96] = "127.0.0.1";
static int             g_port = 8096;
...
static void refresh_stats(void)
{
    ...
    if (body) { body += 4; pthread_mutex_lock(&lk); snprintf(g_fstats, sizeof g_fstats, "%s", body); pthread_mutex_unlock(&lk); }
}
...
int fus_start(const char *host_port)
{
    ...
    fprintf(stderr, "fus: consuming fusion SSE at %s:%d/stream\n", g_host, g_port);
    return 0;
}
...
int fus_connected(void) { return g_connected; }
...
{
    pthread_mutex_lock(&lk);
    int n = snprintf(buf, cap, "%s", g_fstats[0] ? g_fstats : "{}");
    pthread_mutex_unlock(&lk);
    return n;
}

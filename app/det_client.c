/* EO-detector SSE client — subscribes to the detection daemon's /stream (detection/
 * module, :8094) and keeps the latest det message. See det.h. Reconnects forever;
 * reports connected=0 while the daemon is unreachable (the app must run fine with no
 * detector). Same structure as radar_client.c — both are localhost SSE consumers. */
#define _GNU_SOURCE
#include "det.h"
#include <arpa/inet.h>
#include <errno.h>
...
#define ACC_CAP   (256 * 1024)   /* det messages carry up to 128 boxes */
...
static char            g_dstats[STATS_CAP] = "";
...
static void refresh_stats(void)
{
    ...
    if (body) { body += 4; pthread_mutex_lock(&lk); snprintf(g_dstats, sizeof g_dstats, "%s", body); pthread_mutex_unlock(&lk); }
}
...
int det_start(const char *host_port)
{
    ...
    fprintf(stderr, "det: consuming detector SSE at %s:%d/stream\n", g_host, g_port);
    return 0;
}
...
int det_connected(void) { return g_connected; }
...
{
    pthread_mutex_lock(&lk);
    int n = snprintf(buf, cap, "%s", g_dstats[0] ? g_dstats : "{}");
    pthread_mutex_unlock(&lk);
    return n;
}

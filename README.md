...
| Record & replay (`recorder/`) | 🟡 records the whole mission to the NVMe — full-resolution camera, radar, **detections**, and all metadata — without slowing the live system or dropping frames, and replays it looking just like the live screen: full detail, smooth, with the **radar scope and detection boxes in sync** and pause/step/scrub. Recordings can be browsed, tagged, and copied to a laptop. **On-device with the real camera + radar: EO/radar/detection recording verified, native replay smooth, wired into the console replay view.** Next: console record-button polish and field offload — see [`recorder/`](recorder/README.md) |

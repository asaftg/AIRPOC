...
| Radar | ✅ Working on the hardware — detects moving vehicles, drones, and people by range and speed, out to long range, at 26 Hz with no dropped data. Shows each as a target box on a live radar display and feeds them to the operator console. **Next:** sharpen the target's direction so radar alone can aim the head when the camera can't see (haze/night) — see [`radar/`](radar/README.md) |
| Record & replay (`recorder/`) | 🟡 records the whole mission to the NVMe — full-resolution camera, radar, and all data — without slowing the live system or dropping frames, and plays it back looking just like the live screen: full detail, smooth, with pause/step/scrub. Recordings can be browsed, tagged, and copied to a laptop. **Working and tested on-device with the real camera and radar.** Next: the record/replay buttons in the operator console — see [`recorder/`](recorder/README.md) |

...
| Fusion (`fusion/`) | 🟡 `fusiond` on :8096 — joins the radar tracker and the EO tracker into **one target picture**: a fused row carries the radar's range with the camera's angles and class under a global id, unmatched targets pass through source-tagged, and a target never appears twice. It also owns the radar↔EO mount trim. **Console + launcher wiring done** (`/fus/stream`, `/fstats`, `fus_*` knobs, fusion-driven target list with per-sensor fallback, fused symbology on the video, `fusiond` in START/STOP). Open: build + first run on the Jetson, and the mount-trim calibration — see [`fusion/`](fusion/README.md) |

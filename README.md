...
| Record & replay (`recorder/`) | 🟡 standalone C daemon on `:8093`, **HW-verified on the NVMe: native Y10 (10-bit packed, lossless) + display JPEGs + radar + metadata at ~125 MB/s, 0 drops**, crash-safe AIREC sessions, library + timeline replay API. Pending: EO/radar tap tees + console UI (work orders in [`recorder/docs/`](recorder/docs/GUI_INTEGRATION.md)) — see [`recorder/`](recorder/README.md) |

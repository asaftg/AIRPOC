...
| Fusion (`fusion/`) | 🟡 `fusiond` on :8096 — joins the radar tracker and the EO tracker into **one target picture**: a fused row carries the radar's range with the camera's angles and class under a global id, unmatched targets pass through source-tagged, and a target never appears twice. It also owns the radar↔EO mount trim. **Console + launcher wiring done, running on the Jetson, and confirmed fusing real targets** (in replay of session `20260722T195323Z`: parked vehicles fused from ~23 s on). `/fus/stream` + `/fstats` + `fus_*` knobs live, fusion-driven target list (live and replay) with per-sensor fallback, fused symbology on the video, all four wires replay. Open: mount-trim calibration against a live mover — see [`fusion/`](fusion/README.md) |

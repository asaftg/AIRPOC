...
detection/     EO object detector: TensorRT model + track-before-detect (C/CUDA/C++)
...
| Detection (`detection/`) | 🟡 EO object detector live on :8094 — TensorRT model at native 1440×1088 feeding **track-before-detect temporal integration**: a confidence threshold destroys everything below it, so weak evidence is now integrated *before* the gate and a far/low-contrast target that the model only ever half-sees can still be reported (flagged `tbd`), while anything already above threshold is emitted immediately and unchanged. One box per target. Offline on a 30 s day clip this took the detector from **3.5 boxes/tick and zero humans** to 17.7 and humans found — **validated offline only, not yet on the Jetson**. Running a **stock COCO placeholder** (raw-head FP16 ~20 ms / INT8 ~14.7 ms on-device); trained mono model + accuracy pending (data/training agent) — and integration amplifies a weak model's persistent mistakes too, so the model is the real bottleneck. The CPU motion worker is **frozen** (it flooded on foliage and absorbed slow targets), kept only because it is the one path that could see a target the model scores at zero — see [`detection/`](detection/README.md) |
